...
class BedGeneLikeTrack(BedLikeTrack):
    SUPPORTED_ENDINGS = []
    TRACK_TYPE = None
    OPTIONS_TXT = BedLikeTrack.OPTIONS_TXT + """
...
"""
    ...
    DEFAULTS_PROPERTIES = dict({'fontsize': 12,
                                'labels': True,
                                'style': 'flybase',
    ...
    SYNONYMOUS_PROPERTIES = dict({'display': DISPLAY_BED_SYNONYMOUS},
                                 **BedLikeTrack.SYNONYMOUS_PROPERTIES)
    POSSIBLE_PROPERTIES = dict({'style': ['flybase', 'UCSC', 'tssarrow'],
                                'display': DISPLAY_BED_VALID},
                               **BedLikeTrack.POSSIBLE_PROPERTIES)
    BOOLEAN_PROPERTIES = ['labels', 'global_max_row',
                          'arrowhead_included', 'all_labels_inside',
                          'labels_in_margin'] + BedLikeTrack.BOOLEAN_PROPERTIES
    STRING_PROPERTIES = ['style', 'color_utr', 'display'] + \
        BedLikeTrack.STRING_PROPERTIES
    FLOAT_PROPERTIES = dict({'max_value': [- np.inf, np.inf],
                             'min_value': [- np.inf, np.inf],
                             'fontsize': [0, np.inf],
                             'height_utr': [0, 1]},
                            **BedLikeTrack.FLOAT_PROPERTIES)
    INTEGER_PROPERTIES = dict({'gene_rows': [0, np.inf],
                               'max_labels': [0, np.inf],
                               'arrow_interval': [1, np.inf],
                               'arrow_length': [0, np.inf]},
                              **BedLikeTrack.INTEGER_PROPERTIES)

    def set_properties_defaults(self):
        BedLikeTrack.set_properties_defaults(self)
