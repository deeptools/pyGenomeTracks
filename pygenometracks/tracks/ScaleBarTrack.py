...
# x_center, x_boundary1, x_boundary2, size
...
file_type = {TRACK_TYPE}
    ...
    INTEGER_PROPERTIES = {'x_center': [0, np.inf],
                          'x_boundary1': [0, np.inf],
                          'x_boundary2': [0, np.inf],
                          'size': [0, np.inf]}
    ...
    def set_properties_defaults(self):
        ...
        # If the x_boundary2 is set but not x_boundary1 we use x_boundary1:
        if self.properties['x_boundary1'] is None and self.properties['x_boundary2'] is not None:
            self.properties['x_boundary1'] = self.properties['x_boundary2']
            self.properties['x_boundary2'] = None
        # Check that all parameters for position are compatible and set x_center and size:
        param_set = [param for param in ['x_center', 'size', 'x_boundary1', 'x_boundary2']
                     if self.properties[param] is not None]
        if len(param_set) == 4:
            # There is a risk of incompatibility
            computed_size = abs(self.properties['x_boundary2'] - self.properties['x_boundary1'])
            assert self.properties['size'] == computed_size, \
                f"`size` ({self.properties['size']}) is incompatible with "\
                f"`x_boundary1` and `x_boundary2`, should be {computed_size}"
            computed_center = (self.properties['x_boundary1'] + self.properties['x_boundary2']) / 2
            assert self.properties['x_center'] == computed_center, \
                f"`x_center` ({self.properties['x_center']}) is incompatible with "\
                f"`x_boundary1` and `x_boundary2`, should be {computed_center}"
            ...
            if self.properties['x_center'] is None:
                ...
                self.properties['x_center'] = (self.properties['x_boundary1'] + self.properties['x_boundary2']) / 2
                ...
                self.properties['size'] = abs(self.properties['x_boundary2'] - self.properties['x_boundary1'])
            else:  # self.properties['x_boundary2'] is None:
                computed_size = 2 * abs(self.properties['x_boundary1'] - self.properties['x_center'])
                assert self.properties['size'] == computed_size, \
                    f"`size` ({self.properties['size']}) is incompatible with "\
                    f"`x_boundary1` and `x_boundary2`, should be {computed_size}"
    ...
    def plot(self, ax, chrom_region, start_region, end_region):
        ...
        if x_center is None:
            ...
            if self.properties['x_boundary1'] is not None and self.properties['x_boundary2'] is not None:
                # I deduce x_center:
                x_center = (self.properties['x_boundary1'] + self.properties['x_boundary2']) / 2
                # I add size to properties:
                self.properties['size'] = abs(self.properties['x_boundary2'] - self.properties['x_boundary1'])
            elif self.properties['x_boundary1'] is not None and self.properties['size'] is not None:
                # Check if the x axis is decreasing:
                xlim = ax.get_xlim()
                if xlim[0] < xlim[1]:
                    x_center = self.properties['x_boundary1'] + self.properties['size'] / 2
                else:
                    x_center = self.properties['x_boundary1'] - self.properties['size'] / 2
            else:
        ...
        if size is None:
            # Try to guess it from other parameters:
            if self.properties['x_boundary1'] is not None:
                size = 2 * abs(x_center - self.properties['x_boundary1'])
            else:
        ...
        # We now draw the |-----|
        x_boundary1 = x_center - size / 2
        x_boundary2 = x_center + size / 2
        xdatas = [(x_boundary1, x_boundary1),  # vertical line 1
                  (x_boundary1, x_boundary2),  # horizontal line
                  (x_boundary2, x_boundary2)]  # vertical line 2
        ydatas = [(0, 1),  # vertical line 1
        ...
        if where in ['top', 'bottom']:
            ...
            if where == 'left' and xlim[0] < xlim[1]:
                ax.text(x=x_boundary1 - 2 * small_space, y=0.5, s=size_label,
                        horizontalalignment='right',
                        verticalalignment='center', fontproperties=fp)
            elif where == 'left' and xlim[0] >= xlim[1]:
                ax.text(x=x_boundary2 + 2 * small_space, y=0.5, s=size_label,
                        horizontalalignment='right',
                        verticalalignment='center', fontproperties=fp)
